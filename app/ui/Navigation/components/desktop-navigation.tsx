...
const DesktopNavigation = () => {
	...
	return (
		<div
			className={`hidden navigation-blur lg:flex w-full z-50 left-0 fixed top-0 p-4 ${isDark
			...
			<div className='flex max-w-content w-full mx-auto justify-between'>
				...
				<div className='text-gray-800 dark:text-white flex flex-row items-center gap-6'>
					{NAVIGATION_ITEMS.map((item) => (
						<div className={`relative`} key={item.label}>
							<Link
								...
								href={item.url}
								className={
									(pathname === '/' ? pathname.includes(item.url) : pathname === item.url)
										? 'text-green-500'
										: ''
								}
							...
							>
								{item.label}
								<div
									className={`absolute bottom-0 left-0 h-px ${hoveredItem === item.label
										? 'w-full'
										: 'w-0'
					...
					<Link href='https://github.com/Janzunec' target='_blank' rel='noreferrer'>
						<FaGithub className='text-3xl hover:text-green-400 transition-all' />
					</Link>
					<ToggleThemeButton />
				</div>
			</div>
